		...
		if t.st.d[i].Type > TypeStructHead {
			if t.st.d[i].Dim[0] > 0 {
				...
				for k := 0; k < t.st.d[i].Dim[0]; k++ {
					structToHTML(&t.st.d[i], data[t.st.d[i].offset+off:t.st.d[i].offset+off+ln*t.st.d[i].Dim[0]], k, true, prevName+t.PathString(n)+".", &val)
				}
			} else {
				val.WriteString("<td><table><tr><th>Nazwa</th><th>Typ</th><th>Wartość</th></tr>")
				structToHTML(&t.st.d[i], data[t.st.d[i].offset+off:t.st.d[i].offset+off+ln], 0, false, prevName+t.PathString(n)+".", &val)
			}
